...
def read_set(self, filename, header_max = 10000):
    '''
    Reads catalogs on Alberto's format. v4c. 29/01/2013
    '''    
...
#    self.table_name = 'catalog'
    ...
    ###### HEADER 1: Photometric Zeropoints, BPZ corrections, Galactic Extinction, Limiting Magnitudes and FWHM  #######
    i_header += 1
    if not lines[header_divs[i_header][0]].startswith('## FILTER'): raise Exception('Error reading catalog %s header no. %s' % (filename, i_header))
    ...
    for line in l:
        ...
        aux1[cols[1]] = cols[2] #ZP
        aux2[cols[1]] = cols[3] #ZPCor
        aux3[cols[1]] = cols[4] #GalExt
        aux4[cols[1]] = cols[5] #LimMags
        aux5[cols[1]] = cols[6] #FWHM
    header['zero_point'] = aux1
    header['zero_point_correction'] = aux2
    header['extinction'] = aux3
    header['limiting_magnitudes'] = aux4
    header['fwhm'] = aux5
    
    ###### HEADER 2: Columns Description #######
    i_header += 1
...
def read_set_old(self, filename, header_max = 10000):
    '''
    Reads catalogs on Alberto's format. v4b
    '''    
